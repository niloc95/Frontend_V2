window.addEventListener('scroll', function () {
    ...
    if (window.scrollY > 0) {
        ...
        navbar.querySelectorAll('.navlinks-type').forEach(function(link) {
            link.style.transition = 'color 1.2s ease-in-out'; // Add easing to the text color transition
            link.style.color = '#000000'; // change text color to black
        });
        menuIcon.style.transition = 'stroke 1.2s ease-in-out'; // Add easing to the menu icon transition
        menuIcon.style.stroke = '#000000'; // change menu icon color to black

        logoType.style.transition = 'color 1.2s ease-in-out'; // Add easing to the logo-type color transition
        logoType.style.color = '#000000'; // change logo-type color to black

        logoTypeSubType.style.transition = 'color 1.2s ease-in-out'; // Add easing to the logo-type-sub-type color transition
        logoTypeSubType.style.color = '#000000'; // change logo-type-sub-type color to black
    } else {
        navbar.style.transition = 'background-color 1.2s ease-in-out'; // Add easing to the background color transition
